...
<div
  ...
  {#if type !== 'area'}
    <input
      ...
      {type}
      {value}
      class="flex-grow w-full md:text-lg leading-6 align-middle
      transition-colors duration-300 bg-ocitanda-beige outline-none"
      {name}
  ...
  {#if errorStack}
    <p
      ...
      class="absolute right-0 flex flex-col text-xs text-ocitanda-brown mb-3
      mx-2">
      {errorStack}
    </p>
  {/if}
